...
def find_metadata(projectID, minio_client):
	for bucket in minio_client.list_buckets():
		if (bucket.name == "project-{prjectID}".format(projectID)):
			for obj in minio_client.list_objects(bucket.name):
				if (obj.object_name == "metadata.tsv"):
					return (True, bucket.name, obj.object_name)
	return (False, "", "")
...
def label_barcodes(barcode_coords, group, runID, projectID, minio_client):
	...
	groups_object = minio_client.get_object("frontend_groups", "groups.tsv")
	metadata_exists, metadata_bucket_name, metadata_obj_name = find_metadata(projectID, minio_client)
	
	# store the file(s) in 2d lists
	groups_tsv = [line.rstrip('\n').split('\t') for line in groups_object]
	metadata_tsv = ([line.rstrip('\n').split('\t') for line in minio_client.get_object(metadata_bucket_name, metadata_obj_name)]
					if metadata_exists else [])
...
def get_coordinates(vis, runID, minio_client):
	...
	with minio_client.get_object("frontend_coordinates", "{vis}Coordinates.tsv".format(vis=vis)) as coordinate_file:
		reader = csv.reader(coordinate_file, delimiter="\t")
		next(reader) # discard header
		for row in reader:
			barcode = str(row[0])
			if barcode in barcode_coords:
				helper.return_error("duplicate barcode entry in Bucket: frontend_coordinates, Object: {vis}Coordinates.tsv".format(vis=vis))
			else:
